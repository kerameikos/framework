	...
	<p:processor name="oxf:unsafe-xslt">
		...
		<p:input name="config">
			<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:kerameikos="https://kerameikos.org/">
					...
					<content-type>
						...
						<xsl:choose>
							<xsl:when test="count($accept-fragments/node) &gt; 1">
								...
								<!-- validate profiles, only linked.art profile for JSON-LD is supported at the moment, to differentiate from the default Kerameikos.org JSON-LD -->
								<xsl:choose>
									<xsl:when test="$accept-fragments/node[starts-with(., 'profile=')]">
										...
										<xsl:choose>
											...
											<xsl:when test="kerameikos:resolve-content-type($accept-fragments/node[1]) = 'json-ld'">												
												<xsl:choose>
											...
											<xsl:otherwise>
												<xsl:value-of select="kerameikos:resolve-content-type($accept-fragments/node[1])"/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
									<xsl:otherwise>
							...
							<xsl:otherwise>
								<!--<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:value-of select="kerameikos:resolve-content-type($content-type)"/>
									</xsl:otherwise>
								</xsl:choose>-->
				...
				<xsl:function name="kerameikos:resolve-content-type">
