        ...
        <p:input name="config">
            <xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
                ...
                <xsl:template match="/">
                    ...
                    <!-- evaluate whether it is the full data dump or not -->
                    
                    <xsl:choose>
                        <xsl:when test="contains($pieces[last()], 'kerameikos.org')">
                            <exec dir="/usr/local/projects/kerameikos/script"
                                executable="./serialize-rdf.py">
                                <arg line="dump {$format}"/>
                            </exec>
                        </xsl:when>
                        <xsl:otherwise>
                            <exec dir="/usr/local/projects/kerameikos/script"
                                executable="./serialize-rdf.py">
                                <arg line="id {$id} {$scheme} {$format}"/>
                            </exec>
                        </xsl:otherwise>
                    </xsl:choose>                    	
                </xsl:template>
